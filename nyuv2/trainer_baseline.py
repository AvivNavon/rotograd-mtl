...
def main(path, lr, bs, device):
    ...
    for epoch in epoch_iter:
        ...
        for j, batch in enumerate(train_loader):
            ...
            z = backbone(train_data)
            train_pred = [heads[i](z) for i in range(3)]
            losses = torch.stack(
                (
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            for k in range(test_batch):
                ...
                z = backbone(test_data)
                test_pred = [heads[i](z) for i in range(3)]
...
if __name__ == "__main__":
    parser = ArgumentParser("NYUv2 - Equal weights")
    parser.add_argument(
        "--data-path",
        type=Path,
        help="path to data",
